...
#define CONTOURS_MAX_SIZE			150			//光斑圆周点的最多个数
...
public:
	...
	/*差帧比较,像素绝对值差均值，均值差绝对值*/
	int framediff(Mat img, Mat last_img);


	static int m_coarsePosThreshold;	//自适应二值化阈值
	static int G_L_THRESHOLD;       //找标准光斑阈值
	static int last_facula[4][2];   //记录上次定位的4个光斑位置，用于此次定位有效性判定
	static int static_para_signal;  //不同帧参数默认而不更新的标志，0为更新，1为默认不变
	static float SPOT_RATIO_THRESHOLD;  //找标准光斑阈值比例
	static int SPOT_GREYVALUE_DIFF;   //标准光斑边缘梯度条件下限值
private:
	/* 对传入图像进行处理 */
