...
int PupilDetectionTest(void)
{
	...
	switch (choice)
	{
		//在此处（VS2013）测试
	case 0:
		testImgPath = "./images/src3/";
		preName = "./images/des3/eye_";
		break;
		//在封装文件中测试
	case 1:
	...
	case 2:
		...
		//视频实时测试
	case 3:
		testVideoPath = "./videos/src/";		// 视频源路径
		preName = "./videos/des/";
...
//	PupilDetection pulDetec;
	...
	start = clock();
	string splotfilename = "./images/resultfile/splot.txt";
	ofstream splotfile(splotfilename);
	int j = 0, static_para_signal = 0;
...
//		cout << "第 " << i << " 张图 : " << imgPath[i] << endl;
		...
		if (i == 0){
			last_img = img;
			static_para_signal = 0; //0要更新参数，1保持参数
		}
		if (j == 5){
			static_para_signal = pulDetec->framediff(img, last_img);
			j = 0;
			last_img = img;
		}
		
		pulDetec->setImg(img);
		if (PD_SUCCEED != pulDetec->DetectPupil(static_para_signal))
		{
			pulDetec->getImg(img);
			//失败则椭圆参数全部置0
...
void PupilDetecVideoTest(string testVideoPath, string preName, string resultFilePreName)
{
	...
	vector<PDEllipse> vecEllipse;
	PDEllipse resultEllipse;
	string splotfilename = "./videos/splotfile/";
	string str;
	int static_para_signal = 0;
	;
	...
	{
		ofstream splotfile(splotfilename + videoPath[i]+".txt");
		imgFullName.clear();
