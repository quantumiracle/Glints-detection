...
int PupilDetection::ImgProcessing(void)
{
	...
	int i4 = 0,i5=0,total=m_img.cols*m_img.rows,count=0;
	float ra = 0;
		...
		for (i5 = 0; i5 < 15; i5++){
			...
			if (m_img.data[i4]>100 + 10 * i5)
				count++;
		}
			if ((float)count /(float) total < 0.1){   //取前百分之一高亮作为阈值
				G_L_THRESHOLD = 100 + 10 * i5;
				break;
			}
	...
	*/
	...
	for (i = 0; i < contoursSize; i++)
	{
      ...
      // cout << contours[i].size() << endl;
		...
		/*去掉多余光斑*/
		int sumx = 0, sumy = 0, r1 = r - 1, r2 = r + 3, avr1 = 0, avr2 = 0;
		for (j = -r1; j <= r1; j++){
		...
		j = 0;
		avr1 = (sumx + sumy) / (4 * r1 + 2);
		avr2 = m_img.data[(y + r2)*width + x] + m_img.data[(y - r2)*width + x] + m_img.data[(y)*width + x + r2] + m_img.data[(y)*width + x - r2];
		avr2 = avr2 / 4;
		/****************/
		//contours[i][1].x = avr1-avr2;//用该位存储该差值,不可，因为下面用此位
		/*****************/
		if (avr1 - avr2 < 20){   /*光斑与周围平均差值达20以上*/ /***********这个60怎么定******/
			contours[i][0].x = -2;
			continue;
		}
		...
		{
			diff += abs( (contours[i][j].y - y)*(contours[i][j].y - y) + (contours[i][j].x - x)*(contours[i][j].x - x) - (r*r) );
		}
	
		diff = diff / (float)(contours[i].size()*r*r*r);    //r^3是倾向找较大光斑，而过大的已由size上限排除
		//cout << diff;
		if (diff < mindiff){
			mindiff = diff;
			spo_x = x;
	...
	if (n_con < 4){
		for (i5 = 0; i5 < 15; i5++){
			...
			if ((float)count / (float)total < 0.4){   //取前百分之四高亮作为阈值
				G_L_THRESHOLD = 100 + 10 * i5;
				break;
			}
		...
		/*********结果输出************/
	stringstream sss;
	int sig2 = 0, dist = 0, max_dist = 0, N_con=0;
	k = 0;
	...
	else if(n_con == 4 && sig == 0){
		/***校验4点保证间距合适**/
		for (i = 0; i < 4; i++){
			for (j = 0; j < 4; j++){
				if ((configure_contours[i][0] - configure_contours[j][0]) * (configure_contours[i][0] - configure_contours[j][0]) + (configure_contours[i][1] - configure_contours[j][1])*(configure_contours[i][1] - configure_contours[j][1])>spo_r * 2200)
				{
					sss << "error! " ;
					sig2 = 1;
					i = 4;   //直接跳出2重循环
