...
int PupilDetection::ImgProcessing(void)
{
	...
	float ra = 0;
		...
		for (i5 = 0; i5 < 15; i5++){
			...
			if ((float)count /(float) total < 0.2){   //取前百分之一高亮作为阈值
	...
	*/
	...
	for (i = 0; i < contoursSize; i++)
	{
       ...
       // cout << contours[i].size() << endl;
		...
		/*****************/
		if (avr1 - avr2 < 20){   /*光斑与周围平均差值达30以上*/ /***********这个60怎么定******/
			contours[i][0].x = -2;
			continue;
		}
		...
		diff = diff / (float)(contours[i].size()*r*r*r);
		//cout << diff;
		if (diff < mindiff){
			mindiff = diff;
			spo_x = x;
	...
	int configure_contours[50][2];  //合格contour坐标记录数组
	for (i = 0; i < contoursSize; i++)
	{
		if (contours[i][0].x < 0)
		{
		...
		cout << "r:" << spo_r << endl;
		/****************/
		if (contours[i][0].y>13500){   //与标准splot最大距离为12000，否则丢弃，此为范围排除  /**********该12000怎么定，眼过偏时会超过此值，太小又会把多余点包含*****/
			//continue;
		}
