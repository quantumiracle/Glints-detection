...
int PupilDetection::ImgProcessing(void)
{
	...
	int i4 = 0,i5=0,total=m_img.cols*m_img.rows,count=0,last_count=0,inc_count=0;
	float ra = 0,avr_inc_count=0;
	/**/
	for (i5 = 255; i5 > 150; i5--){
	    ...
	    if (i5 < 245){    //前10次预热，使阈值稳定，避免刚开始时平均值不稳定的情况造成特殊情况的高阈值
			if (inc_count > 10 * avr_inc_count)   //一次increase为之前平均的10倍以上，判为出光斑区标志
				break;
		}
		if (i5 != 255){
			avr_inc_count = (float)count / (float)(255 - i5);  //平均单位灰度值下降补充的像素点数
		}
		last_count = count;
		//cout << i5 << " n:" << count <<" "<< avr_inc_count << endl;
	}
	...
	for (i5 = 0; i5 < 15; i5++){
			...
			if ((float)count /(float) total < ra){   //取前百分之一高亮作为阈值->ra
	...
	//二值化
	Mat thresholdImg;
	threshold(m_img, thresholdImg, 180, 255, THRESH_BINARY);   /*GL为判光斑阈值*/
...
//	if (showImg)
	...
	*/
	...
	int max_avr = 0,sta_y=0,sta_x=0;
	float sum_diff_avr = -1000;
	...
	for (i = 0; i < contoursSize; i++)
	{
      ...
      // cout << contours[i].size() << endl;
		...
		if (avr_in - avr_out > max_avr){
			sta_y = y;
			sta_x = x;
			max_avr = avr_in - avr_out;
		}
		/****************/
		//contours[i][1].x = avr_in-avr_out;//用该位存储该差值,不可，因为下面用此位
		/***************/
		
		if (avr_in - avr_out < 20){   /*光斑与周围平均差值达20以上*/ /***********这个60怎么定******/
			contours[i][0].x = -2;
			  //记录此差值，用于二轮一次遍历光斑判定
			continue;
		}
		if (avr_in - avr_out < 60){    //标准光斑的差值要达60
			continue;
		}
		...
		if (avr_in - avr_out - 100 * diff>sum_diff_avr){  //越大越好
			sum_diff_avr = avr_in - avr_out - 100 * diff;  //max_avr越大，mindiff越小越好，且*100使二者同量级
			spo_x = x;
			spo_y = y;
			spo_r = r;
	...
	int sig2 = 0, dist = 0, max_dist = 0, N_con=0, sig_i=-1;
	k = 0;
	if (n_con > 10 || n_con <= 1)
	{
		//cout << "ERROR" << endl;
